// LocalStorage keys for RunwayMap
const STORAGE_KEY = "runwaymap_data";
const CURRENCY_KEY = "runwaymap_currency";
...
function exportData() {
  ...
  link.href = blobUrl;
  link.download = "runwaymap-backup-" + new Date().toISOString().split("T")[0] + ".json";
    ...
    try {
      ...
      if (replaceExisting || subs.length === 0) {
        subs = data.subscriptions.map(s => ({
          ...
          name: s.name,
          price: s.price,
          isSeatBased: s.isSeatBased !== undefined ? !!s.isSeatBased : false,
          currency: s.currency || selectedCurrency || "USD",
        ...
        for (let i = 0; i < data.subscriptions.length; i++) {
          ...
          subs.push({
            ...
            isSeatBased: imported.isSeatBased !== undefined ? !!imported.isSeatBased : false,
            currency: imported.currency || selectedCurrency || "USD",
            cycle: imported.cycle || "Monthly",
            url: imported.url || "",
            color: imported.color || "cyan",
            category: imported.category || "Infrastructure"
          });
        }
      }
