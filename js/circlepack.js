...
class CirclePack {
  ...
  _tangentPositions(c1, c2, r) {
    ...
    const r1 = c1.radius + r + 4; // 4px gap
...
function renderCirclePack() {
    ...
    if (container) {
      container.innerHTML = `
        <div class="flex items-center justify-center h-full text-slate-500 text-xs">
          <p>Add SaaS expenses to see the bubble pack</p>
        </div>
      `;
    }
    return;
  ...
  // Clear non-bubble elements like text nodes
  const firstChild = container.firstElementChild;
  if (firstChild && !firstChild.classList.contains("circlepack-bubble")) {
    container.innerHTML = "";
  }
  ...
  positioned.forEach(item => {
    ...
    const logoSize = Math.max(16, Math.min(item.radius * 0.65, 40));

    let content = "";

    const logoHtml = logoUrl ? `
      <img src="${logoUrl}" alt="${item.name}"
        class="object-contain rounded-md shrink-0 mb-0.5"
    ...
    if (showPrice && showName) {
      content = `
        <div class="flex flex-col items-center justify-center gap-0.5 text-center px-1">
          ${logoHtml}
          <div class="font-semibold text-slate-300 truncate max-w-full" style="font-size: ${fontSize}px;">${item.name}</div>
          <div class="font-black text-slate-100" style="font-size: ${priceSize}px;">${formatCurrencyShort(item.cost)}</div>
        </div>
      `;
    } else if (showName) {
      content = `
        <div class="flex flex-col items-center justify-center gap-0.5 text-center px-1">
          ${logoHtml}
          <div class="font-semibold text-slate-200 truncate max-w-full" style="font-size: ${fontSize}px;">${item.name}</div>
        </div>
      `;
    } else if (logoUrl) {
      content = `
        <img src="${logoUrl}" alt="${item.name}"
          ...
          style="width: ${logoSize}px; height: ${logoSize}px;"
          crossorigin="anonymous"
          onerror="this.parentElement.innerHTML='<span style=\\'font-size:${fontSize}px\\' class=\\'font-bold text-slate-300\\'>${item.name.charAt(0)}</span>';" />
      `;
    } else {
      content = `<span class="font-bold text-slate-300" style="font-size: ${fontSize}px;">${item.name.charAt(0)}</span>`;
    }
    ...
    let bubbleEl = container.querySelector(`.circlepack-bubble[data-id="${item.id}"]`);

    if (!bubbleEl) {
      bubbleEl = document.createElement("div");
      bubbleEl.className = "circlepack-bubble absolute cursor-pointer group";
      bubbleEl.setAttribute("data-id", item.id);
      
      bubbleEl.style.opacity = "0";
      bubbleEl.style.left = item.x + "px";
      bubbleEl.style.top = item.y + "px";
      bubbleEl.style.width = size + "px";
      bubbleEl.style.height = size + "px";
      bubbleEl.style.transform = "translate(-50%, -50%) scale(0.9)";
      bubbleEl.style.setProperty("--cell-color", color.accent);
      bubbleEl.innerHTML = bubbleInnerHtml;
      ...
      bubbleEl.addEventListener("click", e => {
        if (window.innerWidth < 500) {
          ...
          tapCount++;
          if (tapCount === 1) {
            if (activeTooltip && activeTooltip !== bubbleEl) {
              activeTooltip.classList.remove("active");
            }
            bubbleEl.classList.add("active");
            activeTooltip = bubbleEl;
  ...
  // Remove old bubbles
  const existingBubbles = container.querySelectorAll(".circlepack-bubble");
